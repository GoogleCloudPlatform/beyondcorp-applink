...
variable "connector_map" {
  description = "Map from connector keys to service account details and additional metadata."
  type        = map(object({ service_account_id : string, service_account_email : string, additional_metadata : map(string) }))
}

variable "connector_pubsub_map" {
  description = "Map from connector keys to pubsub topic & subscription."
  type        = map(object({ subscription : string, topic : string }))
}
